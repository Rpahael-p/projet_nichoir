...
def on_connect(client, userdata, flags, rc):      
    if rc == 0:                                    
        ...
        client.subscribe([(TOPIC_TEXT, 0), (TOPIC_PHOTO, 0)]) # Subscribe to topics
...
def on_message(client, userdata, msg):            
    if msg.topic == TOPIC_TEXT:                         # Received a text message
        try:
    ...
    elif msg.topic == TOPIC_PHOTO:                      # Received a photo
        ...
        print(f"Received message on {msg.topic}, size: {len(msg.payload)} bytes") # Other topics
