...
void Power_Class::begin() {
    ...
    adc1_config_channel_atten(static_cast<adc1_channel_t>(BAT_ADC_CHANNEL), ADC_ATTEN_DB_11);
...
int16_t Power_Class::getBatteryVoltage() {
    ...
    for (uint16_t i = 0; i < ADC_FILTER_SAMPLE; i++) {
        adc_raw_value += adc1_get_raw(static_cast<adc1_channel_t>(BAT_ADC_CHANNEL));
    }
